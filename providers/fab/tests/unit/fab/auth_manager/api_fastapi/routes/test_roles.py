...
class TestRoles:
    ...
    @patch("airflow.providers.fab.auth_manager.api_fastapi.parameters.conf")
    def test_get_permissions_with_order_by(
        self,
        conf_mock,
        mock_get_application_builder,
        mock_get_auth_manager,
        ...
        dummy = PermissionCollectionResponse(
            permissions=[
                ActionResource(action=Action(name="can_edit"), resource=Resource(name="DAG")),
                ActionResource(action=Action(name="can_read"), resource=Resource(name="DAG")),
            ],
            total_entries=2,
        )
        mock_permissions.get_permissions.return_value = dummy
        ...
        with as_user():
            resp = test_client.get("/fab/v1/permissions", params={"order_by": "-action", "limit": 50, "offset": 10})
            assert resp.status_code == 200
            assert resp.json() == dummy.model_dump(by_alias=True)
            mock_permissions.get_permissions.assert_called_once_with(order_by="-action", limit=50, offset=10)
