...
class GlueJobOperator(AwsBaseOperator[GlueJobHook]):
    ...
    def _find_job_run_id_by_task_uuid(self, task_uuid: str) -> str | None:
    ...
    def execute(self, context: Context):
        ...
        if self.resume_glue_job_on_retry:
            ...
            if previous_job_run_id:
                ...
                try:
                    existing_job_run_id = self._find_job_run_id_by_task_uuid(task_uuid)
                    if existing_job_run_id:
                        job_run = self.hook.conn.get_job_run(JobName=self.job_name, RunId=existing_job_run_id)
                        state = job_run.get("JobRun", {}).get("JobRunState")
                        self.log.info(
